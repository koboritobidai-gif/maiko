...
> **運用メモ**: API呼び出し数を抑えるため、返信本文の取得は「直近アクティブな100スレッド」までに
...
## 3. 請求書チェック(Slack「#請求書」)連携

ダッシュボードの「請求書チェック(#請求書)」カードは、経営者が毎月受け取っている送客パートナーの
請求書PDFをSlackの `#請求書` チャンネルから自動で読み取り、アプリの計算値(4.1参照の送客パートナー
費用)と突き合わせる機能です。未設定の間はカード自体が表示されない(機能OFF扱い)ので、
使いたい場合のみ以下を設定してください。

### 3-1. Bot Token Scope の追加

PDFファイルを読み取るには、2章で作成したSlackアプリに追加のスコープが必要です。

1. [Slack API: Your Apps](https://api.slack.com/apps) を開き、2-1で作成したアプリを選択します。
2. 左メニューの「OAuth & Permissions」を開きます。
3. 「Scopes」→「Bot Token Scopes」で「Add an OAuth Scope」をクリックし、`files:read`
   (ファイル情報の読み取り。PDFのダウンロードに必要)を追加します。
4. スコープを追加すると、画面上部に再インストールを促す案内(「reinstall your app」というリンク)が
   表示されるので、それをクリックして内容を確認し「許可する」をクリックします。
5. 再インストール後、「Bot User OAuth Token」の値が**変わっていないか確認**してください。
   通常はスコープ追加だけではトークンは変わりませんが、もし表示されているトークンが今までと
   異なる場合は、`.env.local` の `SLACK_BOT_TOKEN` を新しい値に貼り直してください。

### 3-2. `#請求書` チャンネルへのボット招待

1. Slack アプリで `#請求書` チャンネルを開きます(無ければ新規作成してください)。
2. チャンネルに `/invite @Tobidai Cockpit`(2-1で付けたApp名)を投稿してボットを招待します
   (2-4と同じ、チャンネル詳細画面の「インテグレーション」タブから招待する方法でも可)。

### 3-3. チャンネルIDの確認

1. `#請求書` チャンネルを開き、チャンネル名をクリックして詳細パネルを開きます。
2. パネルの一番下までスクロールすると、チャンネルID(`C0123ABCDEF` のような文字列)が
   表示されています(2-4のチャンネルID確認と同じ方法です)。

### 3-4. 環境変数の設定

`.env.local`(ローカル開発)と、本番運用している場合は Vercel 等のホスティング環境変数の
両方に、3-3で控えたチャンネルIDを設定します。

```
SLACK_INVOICE_CHANNEL=C0789WXYZ01
```

設定後、開発サーバーを再起動(ローカル)またはデプロイし直す(Vercel)と、ダッシュボードに
「請求書チェック(#請求書)」カードが表示されます。取得に失敗した場合は
「請求書チェック(接続エラー・デモ表示)」バッジになり、サーバーログに具体的なエラー内容が
出力されます。

> **運用メモ**: API呼び出し・転送量を抑えるため、投稿日が直近75日以内のPDFのみを対象にし、
> ダウンロードするPDFは最大15件までに制限しています(超過分は件数のみカードの脚注に表示されます)。
> また、スキャン画像のPDF(テキストが埋め込まれていないPDF)は金額を読み取れないため
> 「金額読取不可」と表示されます。
...
ダッシュボードの「送客売上(貰う金額)」セクションは、経営者が別途運用している送客売上シート
(送客パートナー・その他経路含む、翔び台が紹介先企業から**貰う**金額を記録したスプレッドシート。
4章の広告・SNS運用シートや、3章の請求書=**払う**金額とは別の、既存の外部シートです)を読み取ります。
未設定の間はセクション自体が表示されない(機能OFF扱い)ので、使いたい場合のみ以下を設定してください。

### 5-1. サービスアカウントへの共有

1. 1章で作成済みのサービスアカウント(`tobidai-cockpit-reader@static-chiller-503203-q8.iam.gserviceaccount.com`)
   のメールアドレスを、送客売上シートの「共有」設定に**閲覧者**権限で追加します
   (`GOOGLE_SERVICE_ACCOUNT_FILE` / `GOOGLE_SERVICE_ACCOUNT_JSON` は1章のものをそのまま共用します。
   新たに鍵を作る必要はありません)。
2. シートのURL `https://docs.google.com/spreadsheets/d/【この部分】/edit` の `【この部分】` が
   シートIDです(現行運用シートのID: `14TLsLo2t-EaMIzqQN6dovVk0TWY7MyYr2WaFNQMc24g`。差し替え済みの
   場合は最新のものを使ってください)。

### 5-2. タブ構成について

- 入金月ごとにタブを分けて運用しています。タブ名は「2026年6月」のような「YYYY年M月」形式で、これは
  **対象月**を表します(タブ「2026年6月」の内容は6月分の実績で、実際の入金は翌月末=7月末という運用)。
  この形式に一致しないタブ(「支払い」など)は自動的に無視されます。
- 見出し行(会社名/求職者/流入経路/金額 などの列名が並ぶ行)は、シートの先頭付近にタイトル行が
  挿入されていても自動検出します。列の位置がズレても、見出し文字列(会社名/流入経路/金額)を
  含む列を自動的に探すため、シートの列構成そのものを変更する必要はありません。
- 分割払いの案件(例: 24回払いの一部)は、各月タブに「その月に請求する分」がそのまま記載されている
  運用のため、アプリ側はタブごとに単純合計するだけで正しい月次売上になります。特別な設定は不要です。
- アプリは新しい月順に最大6タブまで読み取ります。

### 5-3. 環境変数の設定

`.env.local`(ローカル開発)と、本番運用している場合は Vercel 等のホスティング環境変数の
両方に、5-1で控えたシートIDを設定します。

```
REVENUE_SHEET_ID=14TLsLo2t-EaMIzqQN6dovVk0TWY7MyYr2WaFNQMc24g
```

設定後、開発サーバーを再起動(ローカル)またはデプロイし直す(Vercel)と、ダッシュボードに
「送客売上(貰う金額)」セクションが表示されます。取得に失敗した場合は
「送客売上(接続エラー・デモ表示)」バッジになり、サーバーログに具体的なエラー内容が出力されます。
...
## 7. 営業実績(`#21_ra`・`#22_アポイント報告`)連携

ダッシュボードの「営業実績(#ra・アポイント報告)」セクションは、法人営業(清本・望月)が日々投稿する
社内Slackの2チャンネルから、架電数・商談数・契約数・アポイント獲得数を自動で読み取ります。
未設定の間はセクション自体が表示されない(機能OFF扱い)ので、使いたい場合のみ以下を設定してください。
2章の Slack App(Bot Token Scopes: `channels:history` / `channels:read` / `users:read`)をそのまま使い、
追加のスコープは不要です。

### 7-1. チャンネルの実際の運用

- **`#21_ra`**: 毎日12:00・15:00にワークフローbotが親メッセージ「架電数報告(12時)」
  「架電数報告(15時)」を投稿し、メンバーがそのスレッドの**返信**で架電数を報告します
  (12時分・15時分は1日の合計として自動で合算します)。また、メンバー自身が親メッセージ
  「業務報告)8/17」のような対象日付きタイトルを投稿し、本文(商談数・契約数)は**返信**に
  書き込みます。「業務予定報告)…」(その日の予定であり実績ではない)は自動的に除外されます。
- **`#22_アポイント報告`**: メンバーがアポイントを1件獲得するごとに、親メッセージ
  「アポイント報告)…」を1件投稿する運用です。**トップレベルの1投稿=アポ獲得1件**として数え、
  獲得経路は本文またはスレッド返信の「経路:」表記・括弧書きから読み取ります(読み取れなければ「不明」)。

上記いずれの書式にも当てはまらない投稿があっても、アプリはエラーにせず黙ってスキップします
(読み取れた分だけを集計します)。

### 7-2. チャンネルへのボット招待とチャンネルID取得

1. Slack アプリで `#21_ra` チャンネルを開きます(すでにボットが招待済みの場合はこの手順は不要です)。
2. 2-4 と同じ手順(`/invite @Tobidai Cockpit` をチャンネルに投稿する方法など)でボットを招待します。
3. 2-4 と同じ方法(チャンネル詳細パネルの一番下)でチャンネルID(`C0123ABCDEF` のような文字列)を
   控えます。
4. `#22_アポイント報告` チャンネルについても同様にボットを招待し、チャンネルIDを控えます。

### 7-3. 環境変数の設定

`.env.local`(ローカル開発)と、本番運用している場合は Vercel 等のホスティング環境変数の
両方に、7-2で控えた2つのチャンネルIDを設定します。実際に運用しているチャンネルIDは以下のとおりです。

```
SLACK_RA_CHANNEL=C076E055KGT
SLACK_APPOINTMENT_CHANNEL=C07K9KN3GG5
```

- 2つの環境変数は独立しています。片方だけ設定しても、設定した方だけが有効になります
  (例: `SLACK_RA_CHANNEL` のみ設定した場合、アポ獲得の列は常に0件になります)。
- 設定後、開発サーバーを再起動(ローカル)またはデプロイし直す(Vercel)と、ダッシュボードに
  「営業実績(#ra・アポイント報告)」セクションが表示されます。取得に失敗した場合は
  「営業実績(接続エラー・デモ表示)」バッジになり、サーバーログに具体的なエラー内容が出力されます
  (どちらか片方のチャンネルだけ取得に失敗した場合は、そのチャンネル分だけが空になり、
  もう片方は正常に表示されます)。

> **運用メモ**: API呼び出し数を抑えるため、返信の取得(架電数報告・業務報告・アポイント報告の
> いずれも)は直近のスレッドを優先し、`#21_ra` は最大120スレッド、`#22_アポイント報告` は
> 最大60スレッドまでとしています。通常の投稿量であれば問題になりません。
