...
var CONFIG = {
  ...
  // ───────────────────────────────────────────────────────────
  //    CSV取り込み（60_CsvImport.gs）でも同じマッピングを使うため、
  //    CSVのヘッダー名（日本語）も候補に含めてあります。
  FIELDS: {
    // 友だち登録日時（LINE登録人数のカウント基準）
    createdAt: [
      '作成日時', '友だち登録日時', '登録日時', '友だち追加日時',
      'created_at', 'createdAt', 'created_time', 'add_time'
    ],
    // 表示ステータス（normal / block / hide、CSVでは 有効 / ブロック など）
    status: ['表示ステータス', 'ステータス', '状態', 'status', 'display_status'],
    // 友だち識別子（重複排除用。無ければ行番号で代用）
    id: ['id', 'friend_id', 'friendId', 'line_user_id', 'user_id', '友だちID', 'ID']
  },

  // 集計に含める表示ステータス。
  // KPIの「LINE登録人数」は“その週に登録した人数”なので、
  // 後からブロックされた友だちも含めるのが既存運用と整合します。
  // API（英語）とCSV（日本語）どちらの表記でも通るようにしています。
  COUNT_STATUSES: ['normal', 'block', 'hide', '有効', 'ブロック', '非表示', '通常'],
  ...
  // ───────────────────────────────────────────────────────────
  // 4. CSV取り込み（API連携オプションが無くても使えるルート）
  //
  //    Lステップ管理画面「友だち管理 > CSV操作 > エクスポート」で出したCSVを
  //    Google ドライブの決まったフォルダに置くだけで、以降の集計・転記は自動。
  //    CSVエクスポートは全プランで使えます。
  // ───────────────────────────────────────────────────────────
  CSV: {
    // CSVを置くドライブフォルダのID（フォルダURLの /folders/ 以降）
    folderId: '',

    // フォルダ内の対象ファイルを絞る文字列（空なら全CSVが対象）
    fileNameContains: '',

    // 文字コード。Lステップのエクスポートは環境により Shift_JIS のことがあるため、
    // UTF-8で文字化けした場合は自動で Shift_JIS を試します。
  ...
  // ───────────────────────────────────────────────────────────
  // 5. Googleカレンダー（費用ゼロ・完全自動）
  //
  //    Lステップの予約はすでにカレンダーへ自動連携されているため、
  //    面談予約数・面談数はカレンダーを読むだけで取得できます。
  //    判定はイベント名で行い、既存の運用ルールをそのまま使います。
  // ───────────────────────────────────────────────────────────
  CALENDAR: {
    // 空ならスクリプト実行アカウントのデフォルトカレンダー
    calendarId: 'tobidai.tobidai@gmail.com',

    // 求職者面談を表すイベント名のパターン
    meetingPattern: 'キャリア相談予約|担当A|担当B|Lリーチ経由|電話面談|再面談',

    // 実施済みを表すパターン（このKPI表の「面談数」に対応）
    heldPattern: '面談済',

    // 予約数からも除くパターン（そもそも面談として成立していないもの）
    excludePattern: '面談見送り|サービス利用不可|中止',

    // リスケ等で同一人物が同じ週に複数イベント並ぶ場合、氏名で1件にまとめる
    dedupeByName: true
  },
  ...
  // 定期実行の設定
  TRIGGER: {
    // API連携ルート / CSVルート: 毎週月曜の朝（既存運用「毎週月曜に前週の数字を入力」に合わせる）
    weekDay: ScriptApp.WeekDay.MONDAY,
    hour: 7,

    // カレンダー＋LINE統計ルート: 毎週木曜の朝
    // LINE統計の反映に最大3日かかるため月曜だと前週日曜分が揃わない。
    // 定例MTG（木曜12:00）の直前に最新化できる利点もある。
    freeWeekDay: ScriptApp.WeekDay.THURSDAY,
    freeHour: 8
  }
};
