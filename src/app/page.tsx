...
export default function TodayDashboardPage() {
  ...
  const isCa = role === "ca";
  const myCandidates = isCa ? getCandidatesByCa(profile.memberId) : [];
  const myActiveCandidates = myCandidates.filter((c) => c.stage !== "辞退");

  const orderedBranches = isCa
    ? [
        ...branchPerformance.filter((b) => b.branch.id === "tokyo"),
        ...branchPerformance.filter((b) => b.branch.id !== "tokyo"),
      ]
    : branchPerformance;
