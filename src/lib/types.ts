...
/**
 * 役割。実在の体制(2026年8月時点、経営者申告): 社長=水口(一覧の最下部に表示)、
 * CA=佐藤/竹林/別府/寺本/松永、法人営業=清本/望月、サポート=畠山/福岡。
 * 小堀(経営者本人)は一覧に載せない運用。「代表」「RA」は旧シートとの互換のため残す。
 */
export type Role = "代表" | "社長" | "CA" | "RA" | "法人営業" | "サポート";
...
export interface ReferralInvoice {
  ...
  parseNote?: string;
  /**
   * 所属スレッドの親メッセージ1行目(例:「8月支払い 請求書【月末払い】」)。#請求書は支払い元の
   * 会社ごとにスレッドが分かれる運用(飛び台/MKC・翔び台・STORYの3系統)のため、「会社別」「スレッド別」
   * の集計に使う。スレッド返信ではなくトップレベル投稿由来のPDFの場合は undefined。
   */
  threadTitle?: string;
  /**
   * true の場合、ダッシュボード画面の支出・請求書チェック合計には含めない(経営者の指示:
   * STORY〈別事業〉のスレッド由来の請求書は画面の合計から除外する従来仕様を維持する)。
   * 「AIに聞く」では除外せず、会社別(スレッド別)の合計に含めて回答できるようにする。
   */
